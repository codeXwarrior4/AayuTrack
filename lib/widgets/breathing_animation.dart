...
import 'package:flutter/services.dart'; // <-- Haptic API (no plugin needed)
...
class _BreathingAnimationState extends State<BreathingAnimation>
  ...
  void initState() {
    ...
    )..addListener(_syncHaptic);
  }

  // ---------------------- HAPTIC SYNC (Replaces Vibration Plugin) ----------------------
  Future<void> _syncHaptic() async {
    if (!isRunning) return;

    double v = _controller.value;

    if (_controller.status == AnimationStatus.forward) {
      // Slightly stronger feedback during inhale
      HapticFeedback.mediumImpact();
    } else if (_controller.status == AnimationStatus.reverse) {
      // Softer feedback during exhale
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        GestureDetector(
          onTap: isCompleted ? restart : (isRunning ? stop : start),
          child: CustomPaint(
